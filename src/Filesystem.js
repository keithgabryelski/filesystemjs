...
class FolderEntry {
  constructor(attributes) {
    Object.assign(this, { createdAt: Date.now(), updatedAt: Date.now() }, attributes);
  }
...
export class Folder {
  ...
  toJSON() {
    return {
      folderEntries: [...this.folderEntries].map(([filename, entry]) => [filename, entry.toJSON()]),
    };
  }
...
class Filesystem {
  ...
  get totalSize() {
    return this.listFolderEntries().reduce((total, entry) => total + entry.size, 0);
  }
